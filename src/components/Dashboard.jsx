...
import { premissasDefault, calcularProjecao } from '../data/mockData';

function Dashboard({ onSair }) {
  const [premissas, setPremissas] = useState(premissasDefault);

  // Recalcula toda vez que qualquer premissa muda — é aqui que entra
  // a fórmula real do modelo financeiro no futuro, no lugar do mock.
  ...
  return (
    <div className="tela-dashboard">
      ...
      <div className="grade-graficos">
        <GraficoReceita receitaPorAno={resultado.receitaPorAno} />
        <GraficoEbitda ebitdaPorAno={resultado.ebitdaPorAno} />
      </div>
    </div>
  );
}
